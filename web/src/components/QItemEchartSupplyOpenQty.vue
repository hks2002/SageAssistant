<template>
  <q-item id="EchartSupplyOpenQty">
  </q-item>
</template>
<script>
if (process.env.DEV) {
  require('../mock/SupplyOpenQty')
}
...
export default {
  ...
  methods: {
    doUpdate (supplyCode) {
      this.$axios.get('/Data/SupplyOpenQty?SupplyCode=' + supplyCode)
        .then((response) => {
        ...
        .catch((e) => {
          ...
          this.$q.notify({
            color: 'negative',
            position: 'top',
            message: 'Loading Supply Open Qty Failed!',
      ...
      this.eChart.setOption({
        ...
        toolbox: {
          feature: {
            dataView: {
              optionToContent: () => {
                return jsonToTable(this.dimensions, this.data, 'Supply Open Qty')
              }
            },
            myTool: {
              show: true,
  ...
  mounted: function () {
    this.eChart = echarts.init(document.getElementById('EchartSupplyOpenQty'))
  }
}
</script>

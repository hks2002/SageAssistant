<template>
  <q-item id="EchartSupplyTotalAmount">
  </q-item>
</template>
<script>
if (process.env.DEV) {
  require('../mock/SupplyTotalAmount')
}
...
export default {
  name: 'QItemEchartSupplyTotalAmount',
  props: {
    supplyCode: String,
    dateFrom: String,
    dateTo: String
  ...
  methods: {
    doUpdate (supplyCode, dateFrom, dateTo) {
      this.$axios.get('/Data/SupplyTotalAmount?SupplyCode=' + supplyCode + '&DateFrom=' + dateFrom + '&DateTo=' + dateTo)
        .then((response) => {
    ...
    setEchart () {
      this.eChart.setOption({
        color: EchartColors,
        title: {
          text: 'Total Amount(' + this.dateFrom + '-->' + this.dateTo + ')',
          subtext: '',
          left: 'center'
        },
        ...
        toolbox: {
          feature: {
            dataView: {
              optionToContent: () => {
                return jsonToTable(this.dimensions, this.data, 'Supply Total Amount')
              }
            },
            myTool: {
              show: true,
  ...
  mounted: function () {
    this.eChart = echarts.init(document.getElementById('EchartSupplyTotalAmount'))
  }
}
</script>

      ...
      >
        ...
        <template v-slot:top>
          ...
          <div class="q-table__control">
            ...
            <q-toggle v-model="mergeCell" label="Merge Cells" />
            <q-toggle v-model="showMore" label="Show More" />
          </div>
          <!-- search sales order ---------------------------------------------------------------------------------->
          <q-space />
          <q-input
          ...
          <q-input
            v-model="searchPurchaseReturnDate"
          ...
          <q-tr :props="props">
            ...
            >
              <div>
                ...
                >
                <q-tooltip
                  >Customer:{{ props.row.SO_BPCORD }}<br />Line:{{
                    props.row.SO_SOPLIN
                  }}</q-tooltip
                >
              </div>
            ...
            >
              <div>
                {{ props.row.SO_ITMREF
                }}<q-tooltip>{{ props.row.SO_ITMDESC }}</q-tooltip>
                <div v-show="showMore">
                  {{ props.row.SO_QTY }} {{ props.row.SO_SAU }}
                </div>
              </div>
            </q-td>
            ...
            >
              <div
                v-if="
                  (new Date(props.row.SO_SHIDATP).getTime() -
                    new Date().getTime()) /
                    (1000 * 60 * 60 * 24) <
                  0
                "
              ...
              <div v-else>
                {{ props.row.SO_SHIDATP
                }}<q-tooltip
                  >Request Date:{{ props.row.SO_DEMDLVDAT }}</q-tooltip
                >
              </div>
            </q-td>
            ...
            >
              <div>
                {{ props.row.SO_CREDAT }}
                <div v-show="showMore">{{ props.row.SO_CREUSR }}</div>
              </div>
            </q-td>
            <!--wo---------------------------------------------------------------------------------------->
            <q-td
            ...
            >
              <div>
                {{ props.row.WO_ITMREF }}
                <div v-show="showMore">
                  {{ props.row.WO_BOMQTY }} {{ props.row.WO_BOMUOM }}
                </div>
              </div>
            </q-td>
            ...
            >
              <div>
                {{ props.row.WO_SHIQTY }}
                <div v-show="props.row.WO_ALLQTY" style="display: inline">
                  : {{ props.row.WO_ALLQTY }}
                </div>
              </div>
            </q-td>
            ...
            >
              <div>
                {{ props.row.WO_CREDAT }}
                <div v-show="showMore">{{ props.row.WO_CREUSR }}</div>
              </div>
            </q-td>
            <!--po---------------------------------------------------------------------------------------->
            <q-td
            ...
            >
              <div>
                {{ props.row.PO_POHNUM
                }}<q-tooltip><br />Line:{{ props.row.PO_POPLIN }}</q-tooltip>
              </div>
            </q-td>
            ...
            >
              <div>
                {{ props.row.PO_ITMREF
                }}<q-tooltip>{{ props.row.PO_ITMDESC }}</q-tooltip>
                <div v-show="showMore">
                  {{ props.row.PO_QTYPUU }} {{ props.row.PO_PUU }}
                </div>
              </div>
            </q-td>
            ...
            >
              <div
                v-if="
                  new Date(props.row.PO_EXTRCPDAT).getTime() -
                    new Date(props.row.SO_SHIDATP).getTime() >
                  0
                "
              >
            ...
            >
              <div
                v-if="
                  new Date(props.row.PO_LINOCNDATA).getTime() -
                    new Date(props.row.SO_SHIDATP).getTime() >
                  0
                "
              >
            ...
            <!--pt---------------------------------------------------------------------------------------->
            <q-td key="ptreceipt" q-pa-xs>
              <div>
                {{ props.row.PT_PTHNUM
                }}<q-tooltip>Line:{{ props.row.PT_PTDLIN }}</q-tooltip>
                <div v-show="showMore">
                  {{ props.row.PT_QTYPUU }} {{ props.row.PT_PUU }}
                </div>
              </div>
            </q-td>

            <q-td key="ptatby" q-pa-xs>
              <div>
                {{ props.row.PT_REPDAT }}
      ...
      columns: [
        {
          ...
          sortable: true,
          align: 'center',
          headerClasses: 'bg-primary text-white text-xs'
        },
        {
          name: 'index2',
          label: '#',
  ...
  methods: {
        ...
        .then((response) => {
          ...
          this.data.forEach((row, index) => {
            ...
            if (index === 0) {
              ...
              if (
                this.data[index].SO_YSOH_PJT !==
                this.data[index - 1].SO_YSOH_PJT
              ) {
                projectIndex++
                row.projectIndex = projectIndex
              } else {
    ...
    myFilterMethod(rows, terms, cols, getCellValue) {
      ...
      if (terms.searchCustomer !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            row.SO_BPCORD.indexOf(terms.searchCustomer.toUpperCase()) !== -1
        )
      }
      if (terms.searchSalesProject !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            row.SO_YSOH_PJT.indexOf(terms.searchSalesProject.toUpperCase()) !==
            -1
        )
      }
      if (terms.searchSalesOrder !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            row.SO_SOHNUM.indexOf(terms.searchSalesOrder.toUpperCase()) !== -1
        )
      }
      if (terms.searchSalesPN !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            row.SO_ITMREF.indexOf(terms.searchSalesPN.toUpperCase()) !== -1
        )
      }
      if (terms.searchSalesDeliDate !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            new Date(row.SO_SHIDATP).getTime() -
              new Date(terms.searchSalesDeliDate).getTime() <=
            0
        )
      }
      if (terms.searchSalesSaleDate !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            new Date(row.SO_CREDAT).getTime() -
              new Date(terms.searchSalesSaleDate).getTime() >=
            0
        )
      }
      if (terms.searchPurchaseProject !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            row.PO_POHNUM.indexOf(terms.searchPurchaseProject.toUpperCase()) !==
            -1
        )
      }
      if (terms.searchPurchaseProject !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            row.PO_PJT.indexOf(terms.searchPurchaseProject.toUpperCase()) !== -1
        )
      }
      if (terms.searchPurchasePN !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            row.PO_ITMREF.indexOf(terms.searchPurchasePN.toUpperCase()) !== -1
        )
      }
      if (terms.searchPurchaseVendor !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            (row.PO_BPSNUM + '').indexOf(
              terms.searchPurchaseVendor.toUpperCase()
            ) !== -1
        )
      }
      if (terms.searchPurchaseReturnDate !== undefined) {
        rowsTemp = rowsTemp.filter(
          (row) =>
            new Date(row.PO_EXTRCPDAT).getTime() -
              new Date(terms.searchSalesDeliDate).getTime() <=
              0 ||
            new Date(row.PO_LINOCNDATA).getTime() -
              new Date(terms.searchSalesDeliDate).getTime() <=
              0
        )
      }
    ...
    spanRow() {
      ...
      this.$children[0].$children[0].computedRows.forEach(
        (row, index, rows) => {
          ...
          if (index === 0 || this.mergeCell === false) {
            ...
            if (row.SO_YSOH_PJT === rows[index - 1].SO_YSOH_PJT) {
              ...
              if (
                row.WO_MFGNUM &&
                row.WO_MFGNUM === rows[index - 1].WO_MFGNUM &&
                row.WO_BOMSEQ === rows[index - 1].WO_BOMSEQ
              ) {
                row.displaywo = false

...
const doUpdate = () => {
  ...
  axios
    .get('/Data/TobeReceive?Site=' + props.site)
    .then((response) => {
      data = response.data
      prepareData()
      setEchart()
  ...
  eChart.setOption(
    {
      title: {
        text:
          t('BOMs purchased from suppliers and to be received by ') +
          props.site,
        left: 'center'
      },
      ...
      toolbox: defaultToolbox(
        dimensions,
        data,
        t('BOMs purchased from suppliers and to be received by ') + props.site
      ),
